...
const files = execSync('find src/lib/components -type f -name "*.svelte"')
	.toString()
	.trim()
	.split('\n');
let count = 0;
...
for (const file of files) {
	...
	if (
		content.includes('const isLandscape = $derived(settings?.isLandscape);') ||
		content.includes('const landscape = $derived(settings?.isLandscape);')
	) {
		// Remove the definition
		content = content.replace(
			/[ \t]*const (?:isL|l)andscape = \$derived\(settings\?\.isLandscape\);\n?/g,
			'',
		);
