<script lang="ts">
	...
	let {
		...
		palette,
		settings,
	}: { seed: number; complexity: number; palette: string[]; settings: PlannerSettings } =
		$props();
...
<div class="cover-bg-wrapper">
	<svg
		...
		<rect
			...
			width="200"
			height="200"
			fill={palette[0] || 'transparent'}
			opacity="0.1" />
		<!-- Use CSS filter to give it an outline or shadow effect -->
		<g
			opacity="0.6"
			style="filter: drop-shadow(1px 1px 0px {palette[1] || 'rgba(0,0,0,0.5)'});">
			{#each grid as item}
				<text
					x={item.x}
					y={item.y}
