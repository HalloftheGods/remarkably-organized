<script lang="ts">
	...
	const shapes = $derived.by<ShapeDef[]>(() => {
		...
		if (themeType === 0) {
			...
			result.push({ type: 'circle', r: radius * N + radius * (0.5 + rand()), color: palette[0], width: 0.5 + rand() });
			if (rand() > 0.5) result.push({ type: 'circle', r: radius * N + radius * (1.5 + rand()), color: palette[1 % palette.length], width: 0.2 + rand() });
		}
		else if (themeType === 1) {
			...
			for(let n=0; n<numSeeds; n++) {
				...
				const c = palette[Math.floor(rand()*palette.length)];
				result.push({ type: 'circle_trans', cx: x, cy: y, r: 0.3 + (n/numSeeds)*(1+rand()*2.5), color: c });
			}
		}
		else {
			...
			for(let i=0; i<centers.length; i++) {
				for(let j=i+1; j<centers.length; j++) {
					if (rand() < lineDensity) {
						result.push({ type: 'line', x1: centers[i][0], y1: centers[i][1], x2: centers[j][0], y2: centers[j][1], color: lineColor, width: 0.1 + rand()*0.4 });
					}
				}
			}
			...
			centers.forEach((pt) => {
				const c = palette[Math.floor(rand()*palette.length)];
				result.push({ type: 'circle_trans', cx: pt[0], cy: pt[1], r: r * (0.3 + rand()*1.2), color: c });
			});
		}
...
<div class="cover-bg-wrapper">
	<svg class="cover-bg-svg" viewBox="-100 -100 200 200" preserveAspectRatio="xMidYMid slice">
		<g opacity="0.4" stroke-width="0.3" fill="none">
			{#each shapes as shape}
				{#if shape.type === 'circle'}
					<circle cx="0" cy="0" r={shape.r} stroke={shape.color} stroke-width={shape.width || 0.3} />
				{:else if shape.type === 'circle_trans'}
					<circle cx={shape.cx} cy={shape.cy} r={shape.r} stroke={shape.color} />
				{:else if shape.type === 'line'}
					<line x1={shape.x1} y1={shape.y1} x2={shape.x2} y2={shape.y2} stroke={shape.color} stroke-width={shape.width || 0.3} />
		...
		<g opacity="0.15" stroke-width="0.2" fill="none" transform="rotate(15) scale(1.1)">
			{#each shapes as shape}
				{#if shape.type === 'circle'}
					<circle cx="0" cy="0" r={shape.r} stroke={shape.color} stroke-width={shape.width || 0.2} />
				{:else if shape.type === 'circle_trans'}
					<circle cx={shape.cx} cy={shape.cy} r={shape.r} stroke={shape.color} />
				{:else if shape.type === 'line'}
					<line x1={shape.x1} y1={shape.y1} x2={shape.x2} y2={shape.y2} stroke={shape.color} stroke-width={shape.width || 0.2} />
				{:else if shape.type === 'poly_raw'}
					<polygon points={shape.points} stroke={shape.color} transform="rotate({shape.rot || 0})" />
				{:else if shape.type === 'path'}
					<path d={shape.d} stroke={shape.color} stroke-width={shape.width || 0.2} fill="none" />
				{/if}
			{/each}
		</g>
	</svg>
