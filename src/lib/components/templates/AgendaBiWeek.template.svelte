...
<div class="planner page flex flex-col w-full h-full p-6 box-border gap-6">
	<div class="flex gap-8">
		...
		<label>
			<strong>{!settings?.emojis?.disable ? '🏃 ' : ''}BI-WEEKLY PLANNER / SPRINT LOG</strong>
		</label>
	...
	<div class="flex gap-6 flex-1">
		<!-- Week 1 -->
		<div class="flex-1 flex flex-col border border-[var(--outline)] rounded overflow-hidden">
			...
			<div class="flex flex-col flex-1">
				{#each new Array(7) as _, i (i)}
					...
					<div class="flex-1 flex border-b border-[var(--outline)] min-h-0 {i === 6
							? 'border-b-0'
							: ''}">
						<a
						...
						<div class="flex-1 p-2 flex flex-col gap-1 overflow-hidden">
							{#each dayEvents as event}
								<span class="text-[0.65rem] bg-[var(--outline-low)] border-l-2 border-[var(--outline)] py-[0.1rem] px-[0.25rem] text-[var(--text)] whitespace-nowrap overflow-hidden text-ellipsis">
									{event.name}
								</span>
							{/each}
						</div>
