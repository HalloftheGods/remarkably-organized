...
const files = execSync('grep -rl "orientation === \'landscape\'" src/lib')
	.toString()
	.trim()
	.split('\n');
...
for (const file of files) {
	...
	if (!content.includes('isLandscape')) {
		...
		if (libImportMatch) {
			...
			if (!imports.includes('isLandscape')) {
				content = content.replace(
					libImportMatch[0],
					`import {${imports}, isLandscape} from '$lib';`,
				);
			}
		} else {
		...
		!content.includes("isLandscape} from '$lib'") &&
		!content.includes("isLandscape } from '$lib'")
	) {
	...
	content = content.replace(
		/settings\?\.design\?\.orientation === 'landscape'/g,
	...
	content = content.replace(
		/const isLandscape = \$derived\(isLandscape\(settings\)\);/g,
		'const landscape = $derived(isLandscape(settings));',
	);
	content = content.replace(
		/const isLandscape = isLandscape\(settings\);/g,
		'const landscape = $derived(isLandscape(settings));',
	);
	// also replace `{@const isLandscape = isLandscape(settings)}` with `{@const landscape = isLandscape(settings)}`
	content = content.replace(
		/{@const isLandscape = isLandscape\(settings\)}/g,
		'{@const landscape = isLandscape(settings)}',
	);
